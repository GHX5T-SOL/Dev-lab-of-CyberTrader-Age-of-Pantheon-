...
export const STATUS: StatusBlock = {
  ...
  recentWins: [
    "Codex autonomous ship loop verified current v6 c6f6f07: ship:check passed with safety scan, typecheck, 201/201 Jest tests in 40 suites, and Expo web export after Operator Brief and presentation-direction patches",
    "Nyx/Vex shipped nyx-p1-007 through v6 e3becc7: /home and /terminal now show an Operator Brief with first-profit progress, Heat ladder posture, and one next action to reduce action fatigue and Heat anxiety",
    "Zoro shipped zoro-p1-003 through v6 c6f6f07: data/presentation-direction.ts locks all 11 commodity lanes, 4 AgentOS faction sigil rules, PirateOS/AgentOS/PantheonOS hierarchy, and Palette/Vex/Reel follow-up asset requests",
    ...
    "Nyx/Oracle shipped AgentOS route pressure through v6 37e6151: generated faction missions now apply reward, timer, and Heat consequences, /missions shows compact ROUTE summaries, and Axiom serial reset uses the intro-to-login path",
    ...
    "Hydra completed retention trigger tuning handoff with npm run retention:tuning so Nyx, Oracle, and Vex get deterministic owner actions before the next balance or UX tuning patch",
    ...
    "Talon follow-up repaired npm run regression:monitor for linked worktrees by persisting state through git rev-parse --git-path regression-state.json; latest post-push monitor checked v6 4f8e9f0 and passed typecheck, Jest, and live health",
    "Zyra/Codex hardened the live and local Axiom smokes in v6 commits 5481191, d5a0a83, 98f1623, daa33e9, ff7b7c3, 93096a5, and 4f8e9f0; live smoke and qa:smoke both pass",
  ...
  signals: [
    ...
    },
    {
      label: "v6 deployment",
      state: "green",
      detail: "https://cyber-trader-age-of-pantheon-v6.vercel.app returns HTTP 200, passes npm run health:live, passes npm run qa:axiom:live, and v6 origin/main is verified through c6f6f07 with ship:check safety/typecheck/201 Jest/export evidence plus Zoro presentation-direction coverage and Operator Brief qa:axiom 11/11, responsive 4/4, provenance, and forced regression evidence on e3becc7",
    },
    {
      label: "Build checks",
      state: "green",
