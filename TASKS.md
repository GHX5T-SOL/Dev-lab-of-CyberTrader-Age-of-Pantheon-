...
6. **Oracle** - 1000-seed economy replay harness, launch tuning bands, adversarial stress scenarios, extended endurance replay (oracle-p0-004, 300 ticks), and player archetype strategy reports (oracle-p0-005) are complete; next Oracle work is beta tuning parameter adjustments driven by archetype insights (oracle-p0-006).
...
12. **Zara** - `zara-p1-004` mobile asset optimization queue and `oracle-p0-005` player archetype strategy reports are complete; next Zara work is icon/splash placeholder creation (`palette-p1-002` dependency) or beta tuning follow-up (`oracle-p0-006`).
13. **Zyra** - live deployment health check, responsive QA self-containment, rollback protocol (talon-p1-003), and extended endurance economy replay (oracle-p0-004) are complete; next Zyra work is launchd runner verification follow-up or axiom-p0-001 web QA execution (`zyra-p1-004`).
