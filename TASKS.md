...
- v6 GitHub repo is public, default branch `main`, latest head is `02ea079` (`palette-p1-003`, pushed `2026-04-28`), with implementation capture work in `2d1d03c`, SuperDesign context in `02ea079`, and `5481191` live-smoke hardening before them.
...
- `zara-p1-004` mobile asset optimization queue is complete:
...
- Current checks previously recorded: `npm run health:live`, `npm run qa:axiom:live`, `npm run capture:screenshots`, `npm run ship:check`, `npm run build:web -- --clear`, `npm run typecheck`, `npm test -- --runInBand`, `npx expo export --platform web`, `npm run qa:responsive`, plus browser smoke for intro/login/trading.
