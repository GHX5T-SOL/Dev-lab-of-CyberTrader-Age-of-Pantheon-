...
- v6 GitHub repo is public, default branch `main`, latest verified head is `15308c9` (`kite-p1-003: add Supabase authority migrations`), after `c895318` completed `kite-p1-004` store-safe wallet/token boundaries, `a065fd3` aligned store-media clearance with autonomy policy, `747ff72` completed `kite-p0-002` launch identity recovery, and `2549e8d` completed the `zara-p1-005` asset provenance workflow.
- v6 Vercel deployment returns HTTP 200, passes the v6 `npm run health:live` shell-marker check, and passes `npm run qa:axiom:live` after commit `5481191` hardened the live Chromium smoke away from a stale `networkidle` wait and onto visible boot-shell markers. Latest combined live monitor result before `c895318`/`15308c9`: HTTP 200, Vercel cache HIT, `npm run qa:axiom:live` 1/1 passed, `npm run safety:autonomous` passed, and `npm run regression:monitor` passed typecheck, Jest, and `health:live` at 2026-04-28T17:23:46Z.
