# Task Board Status - 2026-04-27 (PM)
...
- `hydra-p0-001` market swarm simulation plan is complete: v6 `docs/release/hydra-p0-001-market-swarm-simulation-plan.md` defines five NPC market archetypes (whale, hft, faction, rumor, retail), reproducible `swarmSeed + archetypeMix + tickCount` inputs, a determinism contract that re-uses `engine/prng.ts` and `engine/demo-market.ts`, a default Gate A swarm mix, and a metric table mapping each swarm output to `oracle-p0-002` launch tuning, `oracle-p0-006` beta tuning, `oracle-p0-003` economy stress, and `nyx-p0-001`/`nyx-p0-002` first-session/pressure tasks. Implementation gate is a follow-up task; ElizaOS runtime hookup is explicitly out of scope here. Local v6 `npm run typecheck`, Jest (109/109 in 26 suites), and `npx expo export --platform web` pass. axiom-ship-loop run 2026-04-27T13:28:03Z.
...
14. **Hydra** - `hydra-p0-001` market swarm simulation plan is complete; next Hydra work is the `hydra-p1-002` implementation gate that lands `engine/market-swarm.ts`, `npm run swarm:simulate`, and the per-archetype baseline numbers in v6.
