...
function RiskPill({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <View style={{ borderWidth: 1, borderColor: color, paddingHorizontal: 8, paddingVertical: 5 }}>
      <Text style={{ fontFamily: terminalFont, color: terminalColors.muted, fontSize: 8 }}>{label}</Text>
      <Text style={{ marginTop: 2, fontFamily: terminalFont, color, fontSize: 10 }}>{value}</Text>
    </View>
  );
}
