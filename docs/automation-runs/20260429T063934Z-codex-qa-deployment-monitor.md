...
- v6 later advanced to `7d92e7f` (`hydra-p1-003 hydra: add retention tuning handoff`), `6f0b737` (`vex-p1-006 axiom: verify profile dossier captures`), and final head `a6cb172` (`ghost-p1-005 ghost: accept LocalAuthority launch scope`).
- A later local profile-capture patch appeared in `.superdesign/design-system.md`, `app/menu/profile.tsx`, `qa/axiom-web-regression.spec.ts`, `qa/responsive-captures.spec.ts`, screenshot/provenance assets, and `docs/release/vex-p1-006-profile-dossier-polish.md`. Most of that follow-up was moved upstream or stashed by another process. Final v6 worktree still has local edits in `assets/provenance.json` and `docs/release/vex-p1-006-profile-dossier-polish.md`; these were not committed or pushed by this monitor.
...
| `npm run regression:check` | PASS - forced monitors checked `832cabd`, `3c45be8`, `7d92e7f`, `6f0b737`, and final head `a6cb172`, with typecheck, Jest, and live health green |
