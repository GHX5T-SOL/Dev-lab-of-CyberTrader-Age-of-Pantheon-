...
Updated: 2026-04-28 (full autonomy directive active; v6 49c1e49 Expo 54 lockfile, GLCH provenance, and post-push monitor green)
...
- Android store-candidate artifacts must prove `targetSdkVersion >= 35`; the v6 manifest/lockfile now targets Expo SDK 54, but native EAS evidence still has to prove the Android target SDK and Xcode 26/iOS 26 build chain.
...
- v6 commit `49c1e49` repaired the Expo SDK 54 lockfile after Zara's package bump, added tracked GLCH source/optimized art, refreshed 39-asset provenance, and passed `ship:check`, `health:live`, `qa:axiom:live`, and `regression:monitor`.
