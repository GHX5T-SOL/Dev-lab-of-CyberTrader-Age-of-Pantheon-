...
Updated: 2026-04-29 (full autonomy directive active; v6 4a8c078 tutorial copy review after terminal command flow and cyberdeck surface polish)
...
- Extend the completed Phase 2 faction choice and AgentOS unlock baseline with faction mission depth, route-pressure balance, and tutorialized AgentOS stakes.
...
- `nyx-p1-006` / `oracle-p1-012` is complete through v6 `38359ef`, `4f8e9f0`, and docs head `37e6151`: every AgentOS faction contract stage now carries deterministic route reward/timer/Heat pressure; generated faction missions serialize and display route-pressure summaries in the existing `/missions` contract strip; LocalAuthority and SupabaseAuthority expose `applyMissionPressure`; and local mission completion/failure applies route Heat pressure before normal Heat/bounty feedback. Validation: focused faction/mission/local-authority tests, `npm run typecheck`, `npm run ship:check` with 181/181 Jest tests in 37 suites plus Expo web export, `npm run build:web -- --clear`, `npm run qa:axiom` 11/11, and `npm run qa:responsive` 4/4 passed.
...
- v6 commit `1afc137` is the latest verified pushed head after validation-doc updates for persisted terminal limit-order command wiring, cyberdeck surface polish, terminal pressure-window wiring, AgentOS contract chains, limit-order/faction-pressure interfaces, intro transmission polish, privacy policy sync, AgentOS faction selection, Zoro store-media direction approval, Axiom QA hardening, and provenance refresh; validation through code head `d751d68` / docs head `1afc137` includes `limit-orders:check`, `typecheck`, `ship:check`, local smoke, clean-cache web export, local Axiom QA 11/11, responsive QA 4/4, post-push live health/live Axiom, and forced post-push `regression:check`, with prior `3ab5746` live health/live Axiom and prior `7bf5e38` provenance/perf/regression evidence still valid.
