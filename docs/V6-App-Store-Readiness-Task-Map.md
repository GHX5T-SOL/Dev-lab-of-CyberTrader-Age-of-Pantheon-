...
Updated: 2026-04-29 (full autonomy directive active; v6 1631381 terminal pressure flow after AgentOS contract-chain, limit-order/faction-pressure, and intro transmission ship loops)
...
- **AgentOS rank-5 unlock**: faction choice, one-free-switch allegiance, mission bias, reputation hooks, mission contract chains, deterministic limit-order/faction-pressure engine contracts, and terminal pressure-window previews are now live; route map consequences and persisted limit-order commands remain next.
...
- `oracle-p1-010` is complete in v6 `3ab5746`: v6 adds serializable `LimitOrder`, `LimitOrderFill`, and `FactionMarketPressure` interfaces plus deterministic limit-order creation, cancellation, expiry, buy/sell fills, holding/resource updates, and faction pressure price modifiers. `npm run limit-orders:check` covers order serialization, fills, cancellation, impossible sells, expiry, pressure determinism, and pressure-triggered sell execution; terminal UI wiring remains a follow-up.
- `oracle-p1-011` is complete in v6 `1631381`: bound AgentOS factions now derive deterministic 8-tick pressure windows from aligned contact reputation, `/terminal` applies those windows after location/district/flash modifiers, and the trade ticket renders compact pressure-window plus limit-trigger preview rows while avoiding off-authority order mutation. Validation: focused terminal-pressure/limit-order tests, `npm run typecheck`, `npm run ship:check` with 178/178 Jest tests in 36 suites plus Expo web export, `npm run qa:smoke`, and clean-cache web export passed.
...
- v6 commit `1631381` is the latest verified head after terminal pressure-window wiring, AgentOS contract chains, limit-order/faction-pressure interfaces, intro transmission polish, privacy policy sync, AgentOS faction selection, Zoro store-media direction approval, Axiom QA hardening, and provenance refresh; validation includes focused terminal-pressure/limit-order checks, `typecheck`, `ship:check`, local smoke, and clean-cache web export, with prior `3ab5746` live health/live Axiom and prior `7bf5e38` provenance/perf/regression evidence still valid.
