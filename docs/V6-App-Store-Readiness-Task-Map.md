...
- `zyra-p0-002` monitor pass on 2026-04-26 added `npm run health:live` in v6 and documented it at `docs/release/zyra-p0-002-live-health-check.md`; the live deployment returns HTTP 200 and passes the shell-marker check. A follow-up monitor pass pushed `884e4b1`, making `npm run qa:responsive` self-contained by building the web export before Playwright; current local v6 `health:live`, `typecheck`, and responsive QA pass.
