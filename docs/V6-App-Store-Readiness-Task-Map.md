...
- v6 commit `d751d68` is the latest verified head after persisted terminal limit-order command wiring, cyberdeck surface polish, terminal pressure-window wiring, AgentOS contract chains, limit-order/faction-pressure interfaces, intro transmission polish, privacy policy sync, AgentOS faction selection, Zoro store-media direction approval, Axiom QA hardening, and provenance refresh; validation includes `limit-orders:check`, `typecheck`, `ship:check`, local smoke, and forced post-push `regression:check` with live HTTP 200 / Vercel HIT, with prior `1631381` focused terminal-pressure/limit-order clean-cache export, prior `3ab5746` live health/live Axiom, and prior `7bf5e38` provenance/perf evidence still valid.
