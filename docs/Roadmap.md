...
The latest 2026-04-29 Codex autonomous ship loop verified v6 commit `89d1f9a` after profile dossier assertion hardening, `ghost-p1-005` LocalAuthority launch-scope acceptance, profile dossier capture validation, and `hydra-p1-003` retention tuning handoff. LocalAuthority-only launch scope is accepted for release, SupabaseAuthority remains feature-flagged/deferred until a reviewed live Supabase/RLS/privacy/native evidence pass, and release notes now make the online-authority path a future scope decision instead of a launch blocker. Validation is green: `npm run retention:tuning`, `npm run ship:check` with 188/188 Jest tests in 38 suites plus Expo web export, `npm run qa:responsive` 4/4, `npm run qa:axiom` 11/11, `npm run health:live` HTTP 200, and `npm run qa:axiom:live` 1/1.
